...
def eval_xc_ml(xc_code, rho, spin, relativity=0, deriv=1, verbose=None):  ###new DFA
    ''' seperate for transforming the 1st derivatives in each direction to gamma'''
    if spin == 0:
        rho0, dx, dy, dz = rho[:4]
    ...
    rhos = rho01 + rho02  # add the density on alpha spin and beta spin
    N = rhos.shape[0]  # check the number of grids
    ml_in_ = np.concatenate((rho01.reshape((-1, 1)), rho02.reshape((-1, 1)), gamma1.reshape((-1, 1)),
                             gamma2.reshape((-1, 1)), gamma12.reshape((-1, 1))),
                            axis=1)  # Reshape and Joint

    ml_in = torch.Tensor(ml_in_)  # Convert a NumPy Array to a PyTorch Tensor
    ml_in.requires_grad = True  # Request for XC potential

    exc_ml_out = s_nn(ml_in, is_training_data=False)  # NN works

    ml_exc = exc_ml_out.detach().numpy()
    uni = np.power(rhos, 1 / 3) * 0.75 * np.power(3 / np.pi, 1 / 3)  # a factor to constraint
    ml_exc = ml_exc * (uni.reshape(N, 1))

    exc_ml = torch.dot(exc_ml_out[:, 0], torch.pow((ml_in[:, 0] + ml_in[:, 1]), 4 / 3) * 0.75 * np.power(3 / np.pi,
                                                                                                         1 / 3))  # \int \rho \epsilon dr = Exc

    exc_ml.backward()  # trace to ml_in
    grad = ml_in.grad.data.numpy()  # we got V_xc
...
def evaluator(param):
    ...
    for i in range(len(shapeparam)):  #
        if i == 0:
            theparam.append(torch.tensor(param[:shapeparam[i]].reshape(theshape[i])))
        else:
            theparam.append(torch.tensor(param[sum(shapeparam[:i]):sum(shapeparam[:i + 1])].reshape(theshape[i])))
    with torch.no_grad():
        for params, value in zip(s_nn.parameters(), theparam):
            params.copy_(value)
    list(s_nn.parameters())[-1].data *= scale_factor
    s_nn.eval()
    data = np.load('npz/mylist0.npz')  # If there is sufficient memory available, it is not necessary to use a scheme that involves repeated reading.
    dm = [0] * len(mollist)
    rho = [0] * len(mollist)
    myk = [0] * len(mollist)
    e1 = [0] * len(mollist)
    ...
    for i in range(len(mollist)):
        ...
        output[i] = s_nn(myinput[i], is_training_data=False)  # 这部分与参数相关，因此放到不同粒子的循环内部
        uni = torch.pow(myinput[i][:, 0] + myinput[i][:, 1], 1 / 3) * 0.75 * np.power(3 / np.pi, 1 / 3)  # (grids,)
        output[i] = output[i] * uni.unsqueeze(1)
    Loss = criterion(mollist, output, target_data, myexc_0, myweight, rho, dm, myk, e1, ecoul,
                     enuc)
    loss = Loss.detach().numpy()           #we get loss
    if iterindex >= 1:
        if loss < sa.generation_best_Y[-1]:    # find a better points
            scf_loss = 0
            for i in range(len(mollist)):     # calculate the MAE of new DFAs
                s_nn.eval()
                if mollist[i].spin == 0:
                    vadft = dft.rks.RKS(mollist[i])
                    vadft.grids.level = 5
                    vadft = vadft.define_xc_(eval_xc_ml, 'GGA', hyb=0.2, rsh=[0.0, 0.0, 0.2])
                    A = vadft.kernel()
                    scf_loss += abs(A * 627.5095 - target_data[i])
                    if not vadft.converged:
                        iterindex += 1
                        return 999999.9
                else:
                    vadft = dft.uks.UKS(mollist[i])
                    vadft.grids.level = 5
                    vadft = vadft.define_xc_(eval_xc_ml, 'GGA', hyb=0.2, rsh=[0.0, 0.0, 0.2])
                    A = vadft.kernel()
                    scf_loss += abs(A * 627.5095 - target_data[i])
                    if not vadft.converged:
                        iterindex += 1
                        return 999999.9
            print('iterations [{}], trainingloss: {:.4f}'.format(iterindex, scf_loss / len(mollist)))
            myfile = open('nn1so.txt', 'a+')
            myfile.write('iterations [{}], trainingloss: {:.4f}\n'.format(iterindex, scf_loss / len(mollist)))
            myfile.close()
            if (scf_loss / len(mollist)) < scfloss[-1]:
                renew_rho(mollist)
                torch.save(s_nn.state_dict(), 'nn1so/nn1sopara{}.pth'.format(iterindex))
                '''saving the parameters, make sure you have a 'nn1so' directory '''
                valoss = 0
                for i in range(len(valid_mol)):
                    ...
                    if valid_mol[i].spin == 0:
                        ...
                        valoss += abs(B * 627.5095 - valid_ref[i])
                        print(B, vadft.converged)
                print('iterations [{}], validationloss: {:.4f}'.format(iterindex, valoss / len(valid_mol)))
                myfile = open('nn1so.txt', 'a+')
                myfile.write('iterations [{}], validationloss: {:.4f}\n'.format(iterindex, valoss / len(valid_mol)))
                myfile.close()
    iterindex += 1
    thefile = open('iteration.txt', 'a+')
    thefile.write('here is {} times loss,{}\n'.format(iterindex, loss))  # get the post-scf loss
    return loss
...
s_nn = SNN(input_dim, output_dim, hidden, lamda, beta, use_cuda)  # initial structure of nn
# s_nn.to(device)

###################################################################################
'''Prepare for switching back and forth between SA parameters and Torch network parameters.'''
param_shapes = [param.shape for param in s_nn.parameters()]
positions = [torch.empty(shape) for shape in param_shapes]
nppositions = [np.array(positions[i]) for i in range(len(positions))]

theshape = []
for i in nppositions:
    theshape.append(i.shape)

position = []
for t in range(len(positions)):
    position.append(positions[t].reshape(-1))
shapeparam = []
for i in position:
    shapeparam.append(len(i))

criterion = CustomMSELoss()  # yield a class for loss

for param in s_nn.parameters():  # we got a zero start
    param.data.zero_()
params = torch.Tensor(torch.nn.utils.parameters_to_vector(s_nn.parameters()).shape).uniform_(-1, 1)  # a random start
params1 = torch.zeros(params.shape)
param = np.array(params1)
#######################################################################################

renew_rho(mollist)  # initialize density
sa = SA(func=evaluator, x0=param, T_max=1, T_min=1e-9, lb=-25, ub=1)
best_x, best_y = sa.run()
# print('best_x:', best_x, 'best_y', best_y)
plt.plot(pd.DataFrame(sa.best_y_history).cummin(axis=0))
plt.savefig('reE3.png', dpi=1200)
