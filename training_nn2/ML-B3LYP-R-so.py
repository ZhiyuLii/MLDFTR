...
pfname = str(p)+'-'+str(f)
k_set_generator(p, f)
SNc = get_normal_coefficient(p,f ) # max(S(40,27000))* 22.18 =1, Normalization
##########################################################################
'''Absolute post-SCF, here is calculator for R, which calculating only once'''

def Rtraining(rho, weight, coords, j ):
    ...
    R1 = 0
    for i in range(0,f, 300):   #Segmented calculation to avoid memory explosion
        a = int(i)
        if i+300<=f:
            b=f
        else:
            b = int(i+300)
        kr = np.einsum('ik,jk->ij', k_m[a:b], coords1)  # (grid,3)
    ...
    R1 *= 1 / 64 / 64 / 64 * SNc
    np.savez('Rtraining/R{}'.format(j), R1)


def Rvalidation(rho, weight, coords, j ):
    if rho.shape[0] == 2:  # uks
        rho1 = rho[0]
        rho2 = rho[1]
        rho01, dx1, dy1, dz1 = rho1[:4]
    ...
    for i in range(0,f, 300):
        ...
        kr1 =  frfs = frfc = fifs = fifc =0
    R1 *= 1 / 64 / 64 / 64 * SNc
    np.savez('Rvalidation/R{}'.format(j), R1)





def eval_xc_ml(xc_code, rho, spin, relativity=0, deriv=1, verbose=None):  # nn1
    if spin == 0:
        rho0, dx, dy, dz = rho[:4]
...
def ml_in(rho):  #the input of nn1
    if rho.shape[0] != 2:
        rho0, dx, dy, dz = rho[:4]
        gamma1 = gamma2 = gamma12 = (dx ** 2 + dy ** 2 + dz ** 2) * 0.25 + 1e-10
        rho01 = rho02 = rho0 * 0.5
...
def ml_inR(rho, weight, coords, i, m):   #这里需要更新
    if rho.shape[0] == 2:  # uks
        rho1 = rho[0]
        rho2 = rho[1]
        rho01, dx1, dy1, dz1 = rho1[:4]
...
def evaluator(param):
    ...
    theparam = []
    for i in range(len(shapeparam)):  # 由np的一维度数据生成了torch迭代器的结果
        if i == 0:
    ...
    with torch.no_grad():
        for params, value in zip(nn2.parameters(), theparam):
            params.copy_(value)
    list(nn2.parameters())[-1].data *= scale_factor  # 约束
    ...
    for i in range(len(mollist)):
        ...
        output[i] = nn2(myinput[i], is_training_data=False)

        rhoml1[i] = ml_in(rho[i])
        myinput1[i] = torch.tensor(rhoml1[i], requires_grad=True)
        output1[i] = s_nn(myinput1[i], is_training_data=False)

        uni = torch.pow(myinput1[i][:, 0] + myinput1[i][:, 1], 1 / 3) * 0.75 * np.power(3 / np.pi, 1 / 3)  # (grids,)
    ...
    torch.save(nn2.state_dict(), 'nn2so'+pfname+'/para{}.pth'.format(iterindex))
    print('Epoch [{}], Loss: {:.4f}'.format(iterindex, Loss.item()))
    thefile = open('nn2so'+pfname+'.txt', 'a+')
    thefile.write('here is {} times loss,{}\n'.format(iterindex, loss))
    ...
    VALoss = criterion(valid_mol, output2, valid_ref, myexc_0, myweight, rho, dm, myk, e1, ecoul,
                       enuc,D0)
    print('iterations [{}], valoss: {:.4f}'.format( iterindex,VALoss.item()))
    thefile = open('nn2so'+pfname+'.txt', 'a+')
    thefile.write('iterations [{}], valoss: {:.4f}\n'.format(iterindex, VALoss.item()))
    thefile.close()
    iterindex += 1
    return loss
...
def renew_rho(mollist, n=-1):  # default training.    n = 0 for validation density data
    ...
    scfloss.append(scf_loss / len(mollist))  #
    ...
    np.savez('nn1density/mylist{}'.format(n + 1), **my_dict)
...
renew_rho(mollist, -1)
renew_rho(valid_mol, 0)

data1 = np.load('npz5/mylist1.npz')
rho = [0] * len(valid_mol)
myweight = [0] * len(valid_mol)
mycoords = [0] * len(valid_mol)
for i in range(len(valid_mol)):
   ...
   myweight[i] = data1['myweight{}'.format(i)]
   mycoords[i] = data1['mycoords{}'.format(i)]
   Rtraining(rho[i],myweight[i], mycoords[i],i)
data = np.load('npz5/mylist0.npz')
rho = [0] * len(mollist)
myweight = [0] * len(mollist)
mycoords = [0] * len(mollist)
for i in range(len(mollist)):
   ...
   myweight[i] = data['myweight{}'.format(i)]
   mycoords[i] = data['mycoords{}'.format(i)]
   Rvalidation(rho[i],myweight[i], mycoords[i],i)

###############################################################################

param_shapes = [param.shape for param in nn2.parameters()]
...
for param in nn2.parameters():
    param.data.zero_()
params = torch.Tensor(torch.nn.utils.parameters_to_vector(nn2.parameters()).shape).uniform_(-1, 1)  # 随机初始
params1 = torch.zeros(params.shape)
param = np.array(params1)

####################################################################################

sa = SA(func=evaluator, x0=param, T_max=1, T_min=1e-9)
